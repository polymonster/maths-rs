...
- [x] **I4** `lib.rs:412,587,592,719,724,778,795` — Functions take `&Vec<T>` instead of idiomatic `&[T]`
...
- [x] **M1** Doc comment typos throughout:
...
- [x] **M2** — No prelude module; users must import from multiple submodules
- [x] **M3** `mat.rs:309,320,332,344,356` — Multiple Mat2 `From` impl docs say "constructs Mat3"
- [x] **M4** `Cargo.toml:19` — `serde_json` dependency is unnecessary, users bring their own format
- [x] **M5** `Cargo.toml:12` — Uses `license-file` instead of `license = "MIT"` for crates.io
- [x] **M6** — No crate-level docs on conventions (handedness, row/column major, angle units)
- [x] **M7** — Quat missing `#[cfg_attr(feature="hash", derive(Hash))]` unlike vec/mat types
